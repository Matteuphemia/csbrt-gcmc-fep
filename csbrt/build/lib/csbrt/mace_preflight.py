...
def check_committee(report: Report, config) -> None:
    ...
    if calm.is_ood:
        # A committee that disagrees about a relaxed geometry will trip the
        # fallback on every frame, so the run pays for MACE and never uses it.
        status = "fail"
        detail += (
            ". The relaxed pose is already over the threshold: this committee "
            "would fall back on every frame. Either the members are too "
            "dissimilar to be a committee, or the threshold needs calibrating "
            "against an in-distribution trajectory "
            "(MACEUQMonitor.calibrate_thresholds)"
        )
    elif wild.sigma_f_max_ev_per_ang <= calm.sigma_f_max_ev_per_ang:
        status = "warn"
        detail += (
            ". Disagreement does not grow on a distorted geometry, so this "
            "committee carries little information about where the model fails"
        )
    else:
        status = "ok"
    report.add(
