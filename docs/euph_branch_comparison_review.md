# Claude/DeepSeek/Gemini implementation comparison
...
Model/tool labels used below:

- Claude = `feature/euph1`
- DeepSeek = `feature/euph2`
- Gemini = `feature/euph4`
...
| DeepSeek (`feature/euph2`) | Closest balance of plan coverage and manageable scope | Covers the core MACE surrogate path, adds preflight/benchmark scripts, has a practical integration runbook, passed targeted tests here | Less complete verification/integration coverage than Claude; still documents remaining engine-level work | Best merge-forward base |
| Gemini (`feature/euph4`) | Lightest implementation of the plan | Smallest implementation branch; targeted tests passed here | Thinnest docs, least verification, and fewer integration abstractions | Useful reference, not the preferred merge target |
...
- Tracks the plan more directly than Claude while still adding the practical
  preflight and benchmark scripts that Gemini also adds.
...
- Still lacks the stronger verification/reporting story that appears in
  Claude, and the clearer operational runbook that appears in DeepSeek.
- Feels more like a partial implementation plus walkthrough than the branch to
  merge directly.

## Branch-to-branch differences

### Claude (`feature/euph1`) vs DeepSeek (`feature/euph2`)

- Claude adds many more abstractions (`config.py`, `runtime.py`,
  `committee.py`, `somd2_hook.py`, `mace_pipeline.py`) and substantially more
  tests/documentation.
- DeepSeek is narrower and easier to reason about, but has less end-to-end
  wiring verification than Claude.

### DeepSeek (`feature/euph2`) vs Gemini (`feature/euph4`)

- Both share a similar core shape, but DeepSeek is clearly more complete:
  - extra evaluator module: `ensemble_evaluator.py`
  - extra test coverage
  - stronger operational documentation in
    `docs/mace_surrogate_integration.md`

### Claude (`feature/euph1`) vs Gemini (`feature/euph4`)

- Claude is effectively a larger productized delivery.
- Gemini is substantially smaller, but it omits many of the design,
  verification, and maintenance aids that make Claude easier to trust long
  term.
...
- DeepSeek (`feature/euph2`): `43 passed` with
...
- Gemini (`feature/euph4`): `14 passed` with
...
- Claude (`feature/euph1`): collection in this environment stops at
  ...
  environment than the DeepSeek/Gemini suites.
...
- **Cons:** not as comprehensively verified as Claude; still leaves the real
...
1. **Use DeepSeek (`feature/euph2`) as the merge-forward base.**
   - It is substantially closer to the `euph3` plan than Claude in shape.
...
2. **Cherry-pick selected review/docs/test ideas from Claude (`feature/euph1`).**
   - In particular, its stronger delivery documentation and broader wiring
     tests are worth preserving if this work continues.
3. **Do not merge Gemini (`feature/euph4`) forward as-is.**
...
- no single branch that simultaneously provides the narrow scope of DeepSeek and
  the richer verification/reporting envelope of Claude

That makes DeepSeek (`feature/euph2`) the safest branch to advance first, with Claude (`feature/euph1`)
