...
2. **A Microsecond-Scale Autonomous Safety Net:** A dual-Hamiltonian architecture coupled with real-time Uncertainty Quantification (ensemble force variance + geometry guard) detects unphysical/out-of-distribution states and falls back to classical physics via an instantaneous parameter write—eliminating GPU Context rebuilds. *(Measured: switch latency median $2.40\ \mu\text{s}$ on CUDA, min $2.2\ \mu\text{s}$; 100% interception sensitivity on unphysical geometries with 0 false negatives).*
...
4. **Alchemical Binding Accuracy & Campaign Acceleration:** Evaluated on the OpenBind EV-A71 2A protease benchmark (32 compounds, 74 alchemical edges), MACE hybrid reduces $\Delta\Delta G$ RMSE from $1.23\text{ kcal/mol} \to 0.91\text{ kcal/mol}$ ($25.5\%$ reduction), boosts Pearson correlation from $r = 0.084 \to 0.639$ ($7.6\times$ gain), and cuts catastrophic outliers ($>1.5\text{ kcal/mol}$) by $42.9\%$ (from 7 to 4 compounds). On full solvated production complexes ($58{,}893$ atoms), the engine clocks $195\text{--}210\text{ ns/day}$ ($198.3\text{ ns/day}$ measured on RTX 3070), slashing 52-edge campaign wall-clock time by **$52.0\%$ net campaign speedup** ($377.5 \to 181.2\text{ GPU-hours}$ at latest measured rate, or $384.4 \to 184.5\text{ GPU-hours}$ at baseline).
...
| :--- | :--- | :--- | :--- | :--- |
| **1. Hamiltonian Parity** | Classical-limit parity ($\lambda=0$) | $1305.94\text{ kJ/mol}$ | $1305.93\text{ kJ/mol}$ | **$\Delta E = 3.3 \times 10^{-3}\text{ kJ/mol}$** (exact limit; median switch $2.40\ \mu\text{s}$) |
...
| **5. Binding Free Energy ($\Delta\Delta G$)** | OpenBind EV-A71 (32 ligands, 74 edges) | RMSE $1.23\text{ kcal/mol}$, $r = 0.084$, 7 outliers | RMSE **$0.91\text{ kcal/mol}$**, $r = \mathbf{0.639}$, 4 outliers | **$25.5\%$ RMSE cut; $7.6\times$ Pearson $r$ gain; $42.9\%$ fewer outliers** |
| **6. Solvated MD Throughput** | 58k-atom complex & 52-edge campaign | $377.5\text{--}384.4\text{ GPU-hours}$ ($7.26\text{--}7.39\text{ h/edge}$) | $181.2\text{--}184.5\text{ GPU-hours}$ ($3.48\text{--}3.55\text{ h/edge}$) | **$52.0\%$ net campaign wall-clock reduction** ($195\text{--}210\text{ ns/day}$ on CUDA) |
